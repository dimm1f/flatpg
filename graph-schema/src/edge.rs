...
/// Direction of a half-edge, for schemas whose edges have no orientation.
///
/// Every edge is stored as two halves, one on each endpoint, so that either
/// node can look up its incident edges without scanning the whole graph.
/// `UndefDirection` gives both halves the same `Undef` value, so source and
/// destination are never distinguished. Use this as a schema's
/// [`Schema::D`](crate::schema::Schema::D) type when edges are symmetric
/// (e.g. "connected to", "adjacent to").
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum UndefDirection {
    /// The only direction value; each edge stores one half per endpoint.
    Undef,
}

impl EdgeDirectionKind for UndefDirection {
    fn values() -> &'static [Self] {
        const ARRAY: [UndefDirection; 1] = [UndefDirection::Undef];
        &ARRAY
    }
    ...
    fn make_edge(src: NodeRef, dst: NodeRef, _: Self, handle: EdgeHandle) -> EdgeRef {
        EdgeRef {
            src_node_ref: src,
            dst_node_ref: dst,
            handle,
    ...
    fn orient_edge(&self, src: NodeRef, dst: NodeRef) -> (NodeRef, Self, NodeRef, Self) {
        match *self {
            Self::Undef => (src, Self::Undef, dst, Self::Undef),
        }
    }
}

impl ItemFromIndex for UndefDirection {
    fn from_index(index: usize) -> Option<Self> {
        match index {
            0 => Some(Self::Undef),
            _ => None,
...
impl ItemAsStr for UndefDirection {
    fn as_str(&self) -> &'static str {
        match self {
            UndefDirection::Undef => "Undef",
        }
    }
}
